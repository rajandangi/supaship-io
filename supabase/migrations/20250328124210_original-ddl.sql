...
CREATE POLICY "user can update" ON "public"."user_profiles"
AS PERMISSIVE FOR UPDATE
TO public
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

-- RLS policies for posts_contents
...
-- RLS policies for posts_score
CREATE POLICY "all can see" ON "public"."post_score"
AS PERMISSIVE FOR SELECT
TO public
USING (true);

-- RLS policies for posts_votes
...
-- 1. Drops the publication `supabase_realtime` if it exists, along with any dependent objects (CASCADE).
-- 2. Creates a new publication `supabase_realtime` configured to publish changes (insert, update, delete).
-- 3. Adds the `post_score` table to the `supabase_realtime` publication, enabling real-time replication for this table.
-- 4. Commits the transaction to apply the changes.
BEGIN;
